...
class MetricsStateController extends _$MetricsStateController {
  @override
  MetricsEntity build() => const MetricsEntity();

  void reset() => state = const MetricsEntity();
  ...
  void rollbackLast() {
    ...
    if (state.errors > 0) {
      state = state.copyWith(typed: state.typed - 1, errors: state.errors - 1);
