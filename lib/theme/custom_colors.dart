...
class CustomColors extends ThemeExtension<CustomColors> {
    ...
    Color? c(String path) {
        ...
        return colors.entries.firstWhere(
    ...
    Color? camel(String camelKey) {
        return colors.entries.firstWhere(
            (e) => _toCamel(e.key) == camelKey,
            orElse: () => MapEntry('', Colors.transparent),
        ).value;
    }
    ...
    static String _toCamel(String path) {
        final parts = path.trim().split(RegExp(r'[/ ]+')).where((p) => p.isNotEmpty);
        ...
        return ([first, ...rest]).join().replaceAll(RegExp(r'[^a-zA-Z0-9]'), '');
    }
}
